/**
 * FingerprintJS Pro Azure function v0.3.0 - Copyright (c) FingerprintJS, Inc, 2023 (https://fingerprint.com)
...
function resolveClientIp(request, logger) {
...
function prepareHeadersForIngressAPI(request, preSharedSecret, logger) {
...
function matchesBlacklistedHeaderPrefix(headerName) {
    return BLACKLISTED_HEADERS_PREFIXES.some((prefix) => headerName.startsWith(prefix));
}

async function downloadAgent({ httpRequest, logger }) {
    ...
    return new Promise((resolve) => {
        const data = [];
        const request = https__namespace.request(url, {
            method: 'GET',
            headers,
        }, (response) => {
...
function handleIngress({ httpRequest, logger, preSharedSecret }) {
...
function maybeObfuscateVariable(type, value) {
    ...
    async getValueFromProviders(variable) {
        for (const provider of this.providers) {
            try {
                ...
                if (result) {
                    this.logger?.verbose(`Resolved customer variable ${variable} with provider ${provider.name}. Value: ${maybeObfuscateVariable(variable, result)}`);
                    return {
                        value: result,
...
class EnvCustomerVariables {
    constructor(env = process.env) {
        this.env = env;
        this.name = 'EnvCustomerVariables';
    }
    async getVariable(variable) {
...
(function (StatusFormat) {
    StatusFormat["HTML"] = "html";
    StatusFormat["JSON"] = "json";
})(StatusFormat || (StatusFormat = {}));
const STATUS_PATH = 'status';

async function getEnvInfo(customerVariables) {
    const infoArray = await Promise.all(Object.values(CustomerVariableType).map(async (variableType) => {
...
async function handleStatus({ customerVariables, httpRequest, }) {
...
//# sourceMappingURL=fingerprintjs-pro-azure-function.js.map
